...
prod = getProd();
var selector = [];
selector.push({selector: '#Productpage-bestSellersCont1', attr: 'none', pos: 'before'});
selector = JSON.stringify(selector);
height = "auto";
var passBack = [{selectors: selector, height: height}];
passBack = JSON.stringify(passBack);
addGraphBase(passBack);
var passBack1 = [{title: prod, siteName: 'BabyOye', price: getPrice()}];
passBack1 = JSON.stringify(passBack1);
prepareGraph(pidFlipkart, passBack1);

var selector2 = [];
selector2.push({selector: '#productpageConatiner .addToCartBox', attr: 'none', pos: 'before'});
...
title = title.split("(")[0];
  ...
  if($('#productpageConatiner .quickview-inner-rgt').length>0){
    $('#productpageConatiner .quickview-inner-rgt:eq(0)').after('<a target="_blank" title="Compare via Compare Hatke" href="' + urlToFollow + '" ><div class="buy_compare" style="padding: 12px 35px; color: #000; background: #FCCE00; border: 1px solid #000; font-size: 12px; font-weight: bold; width: 12%;margin-left: 11px;margin-top: -20px;display: inline-block;text-align: center;">Compare Prices</div></a>');
  }
  price = getPrice();
  origPrice = price;
