var alertPosition = 439;
var selector2 = [];
selector2.push({selector: '.product-shop .add-to-cart', attr: 'none', pos: 'before'});
...
title = title.split("(")[0];
  ...
  if($('.product-shop .add-to-cart').length>0){
    $('.product-shop .add-to-cart:eq(0)').after('<a target="_blank" title="Compare via Compare Hatke" href="' + urlToFollow + '" ><div class="buy_compare" style="padding: 10px 35px; color: #fff; background: #BE1310; border: none; font-size: 16px; font-weight: bold; margin: 15px 0px;display: inline-block;text-align: center;border-radius: 3px;">Compare Prices</div></a>');
  }
