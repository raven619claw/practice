...
var selector2 = [];
selector2.push({selector: '#RightSideDiv .pricelist:eq(0)', attr: 'none', pos: 'after'});
...
title = title.split("(")[0];
  ...
  if($('#RightSideDiv .pricelist').length>0){
    $('#RightSideDiv .pricelist:eq(0)').after('<a target="_blank" title="Compare via Compare Hatke" href="' + urlToFollow + '" ><div class="buy_compare" style="padding: 10px 12px; color: #fff; background: #86448E; border: none; font-size: 12px; font-weight: bold; width: 25%;text-align: center;margin-top: 5px;display: inline-block;border-radius: 0px;">Compare Prices</div></a>');
  }
  price = getPrice();
  origPrice = price;
