...
function couponCheck(){
  ...
  if(curURL.split('infibeam.com/ShowCart.action').length>1){
    ...
    if(parseInt(getCookie('removedFirst')) == 1){
      ...
      if($('.promo-div').length>0){
        $('.promo-div:eq(0)').after("<a id='couponClick' href='javascript:void();'><img style='margin-left:65px;' src='" + imgURL + "'></a>");
        addToDOM();
...
function getCoupons(){
  if($('#coupons-applied .close-button').length > 0){
    setCookie("coupInProgress", 1, 1);
    setCookie("doneTill", 0, 1);
    setCookie("perSaving", 0, 1);
    setCookie("removedFirst", 1, 1);
    document.getElementById("coupons-applied").getElementsByClassName("close-button")[0].click();
  }
...
function checkSavings(){
  ...
  cpn = coupons.split("~");
  cpn_msg = "";
  if($('#coupons-and-offers-form .promo-error-messages').length > 0){
    cpn_msg = $('#coupons-and-offers-form .promo-error-messages').text().trim();
  }
  else if($('#coupons-applied .promo-names-summary').length > 0){
    cpn_msg = $('#coupons-applied .promo-names-summary').text().trim();
  }
  couponAt = 99;
  setCookie("cpn_msg" + parseInt(getCookie("doneTill")), cpn_msg, 1);
  // alert(getCookie("cpn_msg" + parseInt(getCookie("doneTill"))));

  if($('#coupons-applied .close-button').length > 0){ 
    //some coupon applied
    if($(".promo-names-summary").find(".amount").length > 0){
      savings = $(".promo-names-summary").find(".amount").text().split(",").join("").trim();
