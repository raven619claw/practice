...
var name = $('.main-prod-heading').find('h1').text();
origProd = name;
...
var imgURL = chrome.extension.getURL("hushbabies2.png");
$('.prod_cart').after('<a target="_blank" href=' + url + '><img style="margin-left:-0%;" src=' + imgURL + ' title="Compare via Compare Hatke"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('.prodtxtSale1:eq(1)').text().split("Rs")[1].trim().split(",").join("");
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = $('.prodtxtSale1:eq(1)').text().split("Rs")[1].trim().split(",").join("");
