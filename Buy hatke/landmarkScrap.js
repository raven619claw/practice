...
function sendCurrent(){
  ...
  prod = $('.primary h1').html();
  if(prod.split("<span").length > 1){
    prod = prod.split("<span");
    prod = prod[0];
  }
  else{
    prod = $('.primary h1').text();
  ...
  if(current_status == 0){
    if($('.price-current').length > 0){
      if($('.price-current').text().split("Rs").length > 1)
      {
        myPrice = $('.price-current').text().split("Rs")[1].split(",").join("").trim();
      }
      else
      {
        myPrice = $('.price-current').text().split(",").join("").trim();
      }
    }
    else
    {
...
function returnPID(link){
  ...
  if(link.split('landmarkonthenet.com').length < 2){
    pid = 0;
  }
  if(link == ""){
    pid = 0;
