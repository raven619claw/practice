...
var name = $('#page-heading').find('h1').text().trim();
name = name.split("(")[0];
origProd = name;
...
$('.compare:eq(0)').after('<a target="_blank" href="' + url + '" class="button compare" pname="Sogo SS-083 Trimmer For Men"><i class="i-btn-cmp"></i><span>Compare Prices</span></a>');

if($('#product-price .price').length!=0){
price = $('#product-price .price').text().trim().split("Rs.")[1].trim().split(",").join("");
}
else {
  price = 0;
}
