...
function blinker(){
  ...
  setTimeout("removeEffects()", 1200);
  setTimeout("blinker()",2400);}
  if (cancel==true){elem1.style.background="#fbfbfb";elem1.style.borderColor="#ddd";}
}

var name = $('#primary_block').find('h1').text().trim();
origProd = name;
...
$('#add_to_cart').find('input').after('<a target="_blank" href=' + url + '><input name="Submit" value="Compare Now" class="exclusive"></a>');

var final2send = url.split("products/");
var msgToSend = final2send[1] + "~*~*" + $('#our_price_display').text().split("Rs")[1].trim().split(",").join("");
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
  ...
  var origPrice = $('#our_price_display').text().split("Rs")[1].trim().split(",").join("");
