...
function sendCurrent(){
	...
	if(price != "" && price != 0 && !isNaN(price) && PID != ""){
		curData.push([prod, image, price, cur_url, current_status, PID, 1]);
...
// 	if($('[itemtype="http://schema.org/Product"]').length > 0 || $('[itemtype="http://schema.org/Book"]').length > 0){
// 		if($("h1").length >  0)
// 		{
...
// 			current_status = 0;
// 		}
// 		if($('#product_overview').length > 0 && $('#product_overview .status:eq(0)').attr('content') == 'discontinued.'){
// 			current_status = 2;
// 		}
// 		console.log("oos: "+current_status);
// 		if(current_status == 0){
// 			if($('.marketplaceStartPriceSpan').length > 0){
// 				if($('.marketplaceStartPriceSpan:eq(0)').text().trim() != "0"){
// 					myPrice = $('.marketplaceStartPriceSpan:eq(0)').text().trim();
// 				}
// 			}
// 			if($(".pricing").length >0 ){

// 				myPrice = $(".pricing").find("#price-after-discount:eq(0)").text().trim();
// 			}
// 			if($(".price").length > 0)
// 			{
// 				myPrice = $(".price:eq(0)").text().trim();
// 			}
// 			else
// 			{
// 				myPrice = "0";
// 			}
// 			if($('#price-after-discount .price').length > 0){
// 				myPrice = $('#price-after-discount .price:eq(0)').text().trim();
// 			}
...
//        PID = decodeURIComponent(cur_url);
...
function returnPID(link){
	...
	if(link.split('infibeam.com').length < 2){
		pid = 0;
	}
	if(link == ""){
		pid = 0;
	...
	if($('.cat-options').length > 0){
		...
		for(j=0;j<res_arr.length;j++){
			...
			if(cat_ids == cat_no){
				variants_id = res_arr[j]['id'];
				// console.log("id: "+variants_id);
				variants_price = res_arr[j]['html_sections']['pricing_summary'];
				if(variants_price.split('itemprop="price"').length > 1){
					price = variants_price.split('itemprop="price"');
					price = price[1].trim();
					price = price.split("/>");
					price = price[0].trim();
					price = price.split('content=');
					price_infi = price[1].trim();
					price_infi = filter_price(price_infi);
				...
				variants_oos = res_arr[j]['html_sections']['pricing_summary'];
				if(variants_oos.split('itemprop="availability"').length > 1){
					oos = variants_oos.split('itemprop="availability"');
					oos = oos[1].trim();
					oos = oos.split("/>");
	...
	if($('#promo_message li').length > 0){
		...
		for(i=0;i<sliderLength;i++){
			...
			if($('#promo_message li:eq('+ i +')').length > 0){
				...
				if($('#promo_message li:eq('+ i +')').find(".promo-tnc:eq(0)").attr('oldtitle').length > 0){
