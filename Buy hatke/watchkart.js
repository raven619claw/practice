...
function reportPurchase(){
  ...
  if(curURL.split('watchkart.com/checkout').length>1){
    chrome.runtime.sendMessage({processDONE: "WatchKart"}, function(response) {
    });
  }
}
...
function sendPairs(){
  arrayToSend = [];
  if($s('.product-list li').length > 0){
    var slider = $s('.product-list li');
    ...
    for(i=0;i<sliderLength;i++){
      PID = "";
      price = "";
      if($s('.product-list li:eq('+ i +')').find('a').length > 0){
        link = $s('.product-list li:eq('+ i +')').find('a:eq(0)').attr('href');
        if(link != ""){ 
          PID = link;
        ...
        if(PID != ""){
          if($s('.product-list li:eq('+ i +')').find('.lsk-price').length > 0){
            price = $s('.product-list li:eq('+  i +')').find('.lsk-price:eq(0)').text();
  ...
  if($s('.prdct-list .product').length > 0){
    var slider = $s('.prdct-list .product');
    var sliderLength = slider.length;
    var link;
    var price;
    var PID;
    ...
    for(i=0;i<sliderLength;i++){
      PID = "";
      price = "";
      if($s('.prdct-list .product:eq('+ i +')').find('a').length > 0){
        link = $s('.prdct-list .product:eq('+ i +')').find('a:eq(0)').attr('href');
        if(link != ""){ 
          PID = link;
          if(link.split("?").length > 1){
            link = link.split("?");
        ...
        if(PID != ""){
          if($s('.prdct-list .product:eq('+ i +')').find('.prcdt-price').length > 0){
            price = $s('.prdct-list .product:eq('+  i +')').find('.prcdt-price').text();
            if(price.split("Rs.").length > 1){
              price = price.split("Rs.");
              price = price[1];
            }
  ...
  chrome.runtime.sendMessage({pairsWatch: arrayToSend}, function(response) {
    });
}
...
function sendCurrent(){
  ...
  if($s('.title .htag').length > 0){
    prod1 = $s('.title .htag').text().trim();
    if($s('.title p').length > 0){
      prod2 = $s('.title p').text().trim();
    }
    prod = prod1+" "+prod2;

  }

  else if($s('[itemprop="name"]').length > 0){
    prod = $s('[itemprop="name"]').text().trim();
  }
  else if($s('.title h1').length > 0){
    prod = $s('.title h1').text().trim();
  }
  if($s(".out-of-stock").length > 0){
    current_status = 1;
  }
  else{
    current_status = 0;
  ...
  if(current_status == 0){
    if($s('meta[itemprop="price"]').length > 0){
      myPrice = $s('meta[itemprop="price"]').attr('content').split(",").join("").trim();
    }
    else if($s('.price-compare .price').length > 0 ){
      myPrice = $s('.price-compare .price').text();
    }
  }
  else{
    myPrice = "0";
  ...
  image = $s('img[itemprop="image"]').attr('src');
  cur_url = window.location.href;
  curData.push([prod, image, myPrice, cur_url, current_status, PID]);
  curData = JSON.stringify(curData);
  ////console.log(curData);
  chrome.runtime.sendMessage({curDataWatch: curData}, function(response) {
          });
}
...
var myurl = "http://compare.buyhatke.com/addWatchList.php";
 var name = $s('h1').text().trim();
          var price = $s('meta[itemprop="price"]').attr('content');
...
var parameters =  encodeURIComponent(prod) + "~*~*" + myPrice + "~*~*" + encodeURIComponent(image2) + "~*~*" + encodeURIComponent(url) + "~*~*67" ;
...
$s('.purchase-btn:eq(0)').after('<br><a target="_blank" href=' + url + '><span class="purchase-btn"><button id="buyFrame2" type="button" title="Compare via Compare Hatke" style="margin-left:85px;" class="button btn-cart"><span><span>Compare</span></span></button></span></a>');
...
siteName2 = "Zivame";
  ...
  class_assigned = "class_naap";
image_name = "http://compare.buyhatke.com/images/unavail_logo.png";
siteName2 = "Others";
}
